...
{ // checks that path is correct
  ...
  for(int i = 0; i < pathCount; i++){
    cout << "Trip#" << tripNum << " Path # " << i << " = " << path[i] << "\n"; // comment this out to avoid printing all paths
...
int main(int argc, char** argv)
{
  ...
  for(int i = 0; i < numTrips; i++)
  { // execute trips with Tripper to find shortest path
